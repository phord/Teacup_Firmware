  ...
  if (next_target.seen_semi_comment == 0 &&
    ...
    if (last_field && (c < '0' || c > '9') && c != '.') {
      switch (last_field) {
        ...
        // case 'K':
        //   if (next_target.option_inches)
        //     next_target.K = decfloat_to_int(&read_digit, 25400);
        //   else
        //     next_target.K = decfloat_to_int(&read_digit, 1000);
        //   if (DEBUG_ECHO && (debug_flags & DEBUG_ECHO))
        //     serwrite_int32(next_target.K);
        //   break;
        case 'R':
          if (next_target.option_inches)
            next_target.R = decfloat_to_int(&read_digit, 25400);
          else
            next_target.R = decfloat_to_int(&read_digit, 1000);
          if (DEBUG_ECHO && (debug_flags & DEBUG_ECHO))
