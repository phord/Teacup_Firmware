...
#undef LOOKAHEAD_MAX_JERK_XY
#define LOOKAHEAD_MAX_JERK_XY 50
#define MAX_JERK_X LOOKAHEAD_MAX_JERK_XY
#define MAX_JERK_Y LOOKAHEAD_MAX_JERK_XY
#define MAX_JERK_Z 0
#define MAX_JERK_E LOOKAHEAD_MAX_JERK_E
void dda_find_crossing_speed(DDA *prev, DDA *current) {
  ...
  else if (prev->delta.X == 0 && prev->delta.Y == 0)
    prev_distance = prev->z_direction ? prev->delta.Z : - prev->delta.Z;
  else
  ...
  else if (current->delta.X == 0 && current->delta.Y == 0)
    curr_distance = current->z_direction ? current->delta.Z : - current->delta.Z;
  else
...
sersendf_P(PSTR("prevF: %ld  %ld  %ld  %ld\n"), prevFx, prevFy, prevFz, prevFe);
sersendf_P(PSTR("currF: %ld  %ld  %ld  %ld\n"), currFx, currFy, currFz, currFe);

  /**
   * What we want is (for each axis):
   *
   *   delta velocity = |v1 - v2| < max_jerk
   *
